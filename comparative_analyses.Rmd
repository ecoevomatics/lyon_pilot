...
## Read in all final data from dada2 and Lyon Pilot blast taxonomy analysis
```{r}
...
## Merge with metadata
```{r}
final <- merge(merged_long_id, metadata, by = "Sample", all.x = TRUE)

## Export merged
write.csv(final, "~/bryo_arthro/lyon_pilot_git/data/merged_for_analyses.csv", row.names = FALSE)
```

## Now we can being analyses on the dataset

## Heatmap of ASV frequency for the highest frequency ASVs
```{r}
library(dplyr)
