...
First remove samples we donʻt want to look at through rarefaction:
...
```{r}
ps4rar_ord <- prune_samples(sample_sums(ps4rar_merged) >0, ps4rar_merged) |> 
    ordinate("NMDS", "bray")
plot_ordination(ps4rar_merged, ps4rar_ord, type="samples", color="site_name", shape="substrate", title="Samples (Distance metric: Bray)")

plot_ordination(ps4rar_merged, ps4rar_ord, type="taxa", color="order", shape="class", title="Samples (Distance metric: Bray)")

plot_ordination(ps4rar_merged, ps4rar_ord, type="", color="order", shape="class", title="Samples (Distance metric: Bray)")
```

Merge ASVs of the same taxa with taxglom
```{r}
